...
def process_email(email_contents: str) -> List[int]:
    ...
    email_contents = re.sub(r'\bhttp\S*', 'httpaddr', email_contents)

    # FIXME: Handle email addresses.
    # Convert all strings with @ in the middle to a 'emailaddr' token.
    email_contents = re.sub(r'[\w\-]+@[\w\.]+\b', 'emailaddr', email_contents)
    ...
    for token in tokens:
        ...
        if len(token) < 1:
            continue
             ...
             for i in range(len(vocabulary_dict)-1):
                  if vocabulary_dict[i+1]==token:
